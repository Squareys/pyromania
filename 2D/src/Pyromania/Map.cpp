...
int Map::load_map(char* filename){
	...
	//Load list of walkable tiles
	walknum  = pack_igetl(mapfile);
	walkable = new int[walknum];
	pack_fread(walkable, walknum*4, mapfile);

	//Load list of destructable tiles
	destrnum = pack_igetl(mapfile);
	destructable = new int [destrnum];
	pack_fread(destructable, destrnum*4, mapfile);
...
bool Map::isDestructable(int x, int y){
	return (tileDestructable(x/tilew, y/tileh));
}


bool Map::tileDestructable(int tx, int ty){
	int tile = tileAt(tx, ty);

	if (tile == 15) return true;

	for ( int i = 0; i < destrnum; i++){
		if (destructable[i] == tile){
			return true;
		}
	} 
	return false;
